...
The price of vertical slicing is target count (3 features × 5 targets). That boilerplate is folded away by 4 local plugins and a DSL.
...
| `CoreDesignSystemDemo` | Shared component demo | 〃 |
| `CoreStorageDemo` | Storage behavior check + hosted-test host | 〃 |
...
| Concurrency | Swift 5, unstructured `Task { }` | Swift 6 strict `complete`, `ManagedTask` (re-entry & deinit cancellation discipline), Sendable boundaries |
...
### Tests (24)
- **Contract tests**: the in-memory Mock and the real Keychain implementation pass the same `TokenStorage` exam. Keychain needs a host app, so those run in a separate `CoreStorageHostedTests` target.
