...
핸들을 잃은 작업은 더는 취소할 수 없습니다. 이 레이스가 위험한 진짜 이유가 여기 있습니다. 그래서 슬롯을 비우는 조건을 "아직 교체되지 않은 작업일 때만"으로 뒀습니다. 시작 시점의 `generation` 값을 let으로 캡처해 두고, 완료 시 현재 값과 일치할 때만 슬롯을 비웁니다.
...
**`generation` 비교가 있는 이유.** 앞에서 본 레이스 때문입니다. 자기 Task 핸들을 캡처해서 비교하는 방식은 Sendable 클로저 안에서 var 캡처를 바꾸는 코드가 되어 Swift 6이 거부합니다. 그래서 시작 시점의 `generation` 값을 let으로 캡처해 비교합니다.
...
이 대비가 문제의 본질을 보여 줍니다. structured든 `.task`든 ManagedTask든 관건은 하나, 비동기 작업의 수명을 누가 무엇에 묶어 주는가입니다. structured는 함수 스코프에, `.task`는 뷰 수명에, ManagedTask는 소유 객체인 ViewModel 수명에 묶습니다. 묶이지 않은 채 방치된 작업만이 버그입니다.

---

## 직접 만드는 게 일반적인가

RxSwift에는 DisposeBag, Combine에는 `Set<AnyCancellable>`이 있는데 Swift Concurrency에는 대응하는 표준 부품이 없습니다. ManagedTask는 그 빈자리를 채운 타입입니다.
...
Swift Concurrency에는 DisposeBag이나 CancelBag에 해당하는 표준 그릇이 없습니다. UIKit에서 `Task {}`를 쓰는 순간 앞의 다섯 규칙이 필요한데 담을 곳이 없어서, 실무는 둘로 갈립니다. 각자 만들거나(TaskBag, CancelBag, SingleTask 등 이름은 제각각이지만 열어보면 거의 같은 모양입니다) VM마다 복붙합니다. 후자가 흔하지만 흔한 것과 좋은 것은 다릅니다.
...
그리고 테스트가 정말 회귀를 잡는지 보려면 직접 깨 봐야 합니다. `generation` 비교 조건을 잠깐 지우고 돌려 빨간불을 눈으로 확인했습니다. 키체인 -34018 실측과 같은 부류의 증명입니다.

---

## 정리

structured concurrency의 보장은 async 함수 안에서만 성립합니다. UIKit의 진입점인 viewDidLoad와 버튼 핸들러는 동기 함수라 async를 시작할 방법이 `Task {}`뿐이고, 이 경계에서 unstructured는 차선이 아니라 정석 진입점입니다.

대신 structured가 자동으로 주던 보장이 사라집니다. 수명 묶기, 재진입 취소, 취소 후 결과 무시. 그 규칙을 ManagedTask라는 타입으로 복원해 컴파일 타임에 강제되게 했습니다.

SwiftUI였다면 뷰가 시작하는 작업은 `.task(id:)` modifier가 프레임워크 차원에서 관리해 줬을 것입니다. 이 설계가 푸는 문제의 본질이 "수명을 누가 묶는가"라는 뜻입니다.

replace와 runIfIdle을 나눈 이유도 여기서 나옵니다. 검색은 새 요청이 이전 요청을 대체해야 하고, 페이징은 진행 중이면 새로 시작하지 말아야 합니다. 동작이 다른데 API가 하나면 호출부가 실수합니다.
