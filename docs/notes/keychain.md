...
`KeychainTokenStorage` 68줄을 뜯어보며 나온 질문들을 정리했다. 왜 UserDefaults가 아니라 Keychain인가, `kSecClassGenericPassword`와 service/account는 무엇인가, delete 후 add는 왜인가, 그리고 접근성 상수 한 줄에 담긴 보안 결정은 무엇인가.

Keychain은 암호화되는 UserDefaults가 아니라 자격증명을 위한 시스템 서비스다. 설계의 핵심도 저장 코드가 아니라 정책 선언에 있다. 이 아이템을 언제 읽을 수 있고 어디까지 따라가는지를 상수 하나로 선언하면 나머지는 OS와 Secure Enclave가 맡는다.

이 레포가 고른 `AfterFirstUnlockThisDeviceOnly`는 백그라운드 네트워킹은 되게 하고 기기를 옮기면 재로그인을 강제하는, 토큰의 표준 답이다.
...
전편인 Clean 버전은 토큰을 UserDefaults에 저장했다. 이 레포는 미루지 않고 바꿨다. 근거는 저장 형식의 차이 그 자체다.
...
여기서 "앱을 지웠는데 로그인이 살아 있다"는 현상이 나온다. 키체인 잔존은 문서화된 보장이 아니라 관찰되는 일반적인 동작이다. 재설치를 초기화로 기대하는 사용자와 어긋날 수 있으니, 실무에서는 첫 실행 플래그를 UserDefaults에 두고 감지해 키체인을 명시적으로 비운다. 두 저장소의 삭제 시점 차이를 역이용하는 방법이다.

### 아이템 식별 - baseQuery가 기본키다
...
키체인 아이템 클래스는 5종이다. `genericPassword`는 범용 비밀로 API 토큰의 관례적 자리고, `internetPassword`는 서버와 프로토콜 메타를 포함하며, 나머지는 `certificate`, `key`, `identity`다.

genericPassword의 유일성은 service와 account의 조합이 결정한다. 같은 조합으로 add하면 `errSecDuplicateItem`이 난다. store에서 delete를 먼저 하는 이유이기도 하다.

init 파라미터로 열어 둔 덕에 데모 앱이 `account: "demo.auth.token"`으로 본 앱과 다른 슬롯을 쓸 수 있다. 실제로 [FeatureAuthDemoApp](../../Projects/Feature/FeatureAuth/Demo/Sources/FeatureAuthDemoApp.swift)이 그렇게 쓴다.
...
SecItemUpdate 경로는 존재를 확인하고 분기해야 해서 코드가 배로 늘어난다. 토큰 저장은 로그인 순간뿐인 저빈도 작업이라 단순함을 샀다. 주석에도 명시한 결정이다. delete와 add 사이의 이론상 빈틈은 로그인 플로우 특성상 동시 store가 없어 실질적으로 무해하다.

값은 항상 Data다. `Data(token.utf8)`이 가장 원초적인 직렬화의 실물이다.

접근성은 add 시점에 선언한다. 저장 후에 바꾸려면 아이템을 다시 써야 한다. 정책은 저장할 때 정해진다.

### 접근성 스펙트럼 - 이 한 줄이 진짜 보안 설계다

`kSecAttrAccessible`의 선택지는 두 축이다. 언제 읽을 수 있나, 그리고 기기를 따라가나.
...
이 레포의 `AfterFirstUnlockThisDeviceOnly`는 잠금 중 백그라운드 요청은 되게 하고, 기기가 바뀌면 자격증명이 따라가지 않게 한다. 접근성 선택은 보안 강도를 높이는 문제가 아니라 앱의 동작 요구에 맞추는 문제다.

막히는 메커니즘도 알아 둘 만하다. 화면이 잠기는 순간 해당 보호 클래스의 복호화 키가 메모리에서 내려간다. 잠금 중 `SecItemCopyMatching`은 `errSecInteractionNotAllowed`로 실패하고, 앱 입장에서는 토큰이 없으니 로그아웃된 것처럼 보이는 침묵 실패가 된다. 백그라운드 fetch, 푸시를 받고 깨어난 처리, 잠긴 채 도는 background URLSession이 전부 이 실패를 겪는 지점이다.

**정직하게 짚으면 이 앱은 지금 WhenUnlocked여도 안 터진다.** GitSearch에는 현재 백그라운드 네트워킹이 없다. `WhenUnlocked`로 저장해도 당장은 아무 문제가 없고 보안 강도는 오히려 한 단계 높다. 잠기면 즉시 재보호되기 때문이다. 그래도 AfterFirstUnlock을 쓴 이유는 이게 나중에 터지는 종류의 함정이어서다. 백그라운드 기능을 추가하는 순간 가끔씩만 로그인이 풀리는, 재현하기 어려운 버그로 돌아온다. 접근성은 저장 시점에 고정되므로 나중에 조용히 어긋나는 것보다 처음부터 토큰의 관례적 표준을 쓰는 편이 낫다고 봤다. 트레이드오프가 있는 선택이지 공짜가 아니다.

ThisDeviceOnly의 논거를 토큰 입장에서 다시 쓰면 이렇다. 토큰이 백업을 타고 다른 기기로 복원되면 그 자체가 유출이고, 어차피 재로그인하면 재발급되는 값이라 백업에 태울 이유가 없다. 잃어도 다시 만들 수 있는 자격증명은 기기에 묶는다.
...
retrieve만 status를 확인하고 store와 clear는 반환값인 OSStatus를 버린다. 교보재로서의 단순화다. 실무라면 이렇게 하면 안 된다.

store의 `SecItemAdd` 실패를 침묵하면 로그인했는데 다음 실행에 풀려 있는 미스터리가 된다. 디스크 문제나 마이그레이션 직후, 잠금 상태처럼 드문 경우지만 최소한 로깅은 해야 하고 이상적으로는 throws로 전파해야 한다.

**침묵 실패를 실제로 재현해 봤다(2026-07-12, 이 코드베이스).** 호스트 없는 테스트 타겟에서 `store()`를 부르면 에러 없이 조용히 지나가고 `retrieve()`가 nil을 준다. OSStatus를 직접 찍어보고서야 `SecItemAdd`가 -34018(`errSecMissingEntitlement`)을 반환한 것이 원인으로 드러났다. 같은 코드가 호스트인 Demo 앱을 주입한 타겟에서는 4개 모두 통과했다. OSStatus를 버리면 원인 진단이 불가능해진다는 것과, 키체인 테스트에 호스트가 필수라는 것을 동시에 보여 준 실험이다. CoreStorage의 Tests와 HostedTests를 분리한 근거이기도 하다.

대표 상태값은 성공인 `errSecSuccess`(0), retrieve에서는 정상 경로인 `errSecItemNotFound`(-25300), delete-first가 회피하는 `errSecDuplicateItem`(-25299), 접근성 위반 시점의 접근인 `errSecInteractionNotAllowed`(-25308)다.

### Sendable과 동기 API

Sendable이 성립하는 근거는 세 가지다. 저장 프로퍼티가 전부 let이고, 실제 상태는 시스템인 Keychain이 보유하며, SecItem API 자체가 스레드 안전하다. class여도 상태의 위치를 물으면 증명이 성립하는 사례다.

동기 API로 둔 이유는 키체인 조회가 로컬이라 빠르기 때문이다. 덕분에 `AuthTokenInterceptor.adapt`가 동기로 남았고 prepareRequest 전체를 네트워크 없이 테스트할 수 있게 됐다. 저장소의 동기와 비동기 결정이 파이프라인 설계까지 흐르는 맞물림이다.

---

## 사용 설명서 - 함수 4개와 상수 사전

키체인 API는 함수 4개가 전부다. 대신 모든 것이 딕셔너리로 표현되기 때문에, 실력은 함수가 아니라 딕셔너리에 어떤 키를 넣을 줄 아는가에서 갈린다.
...
조회는 값만 받을 수도, 속성까지 받을 수도, 전체를 나열할 수도 있다.
...
딕셔너리에 들어가는 키는 네 가지 역할로 나뉜다. 식별자인지, 값인지, 정책인지, 반환 지시인지를 구분하면 어떤 조합도 읽을 수 있다.
...
`kSecAttrAccessible`의 선택지 전부다. 아래로 갈수록 엄격하다.

| 상수 | 읽을 수 있는 시점 | 백업/이전 | 이런 데서 쓴다 |
|---|---|---|---|
| ~~`Always`~~ / ~~`AlwaysThisDeviceOnly`~~ | 항상 (잠금 무관) | 포함/제외 | iOS 12에서 deprecated. 재부팅 직후도 열리는 건 보호가 아니라서 퇴출됐다 |
...
| 래퍼 라이브러리 | KeychainAccess 등 | CFDictionary 보일러플레이트 제거. 단 접근성과 동기화 정책 결정은 라이브러리가 대신 못 한다 |

공통 원리는 하나다. 키체인 설계는 정책 선언의 조합이다. 코드는 CRUD 네 줄이고 실력 차이는 상수 선택에서 난다. 정책 축 네 개를 모으면 이렇다.
...
토큰의 답은 언제는 AfterFirstUnlock, 누가는 내 앱만이라 기본값, 절차는 없음, 어디까지는 이 기기만이다. 이 조합이 백그라운드에서 조용히, 내 앱만, 이 기기에서만이라는 토큰의 사용 프로파일을 그대로 옮겨 놓았다.

### kSecAttrAccessGroup - 어느 앱들이 공유하나

모든 아이템은 접근 그룹에 소속된다. 지정하지 않으면 기본 그룹인 `팀ID.번들ID`, 즉 그 앱 전용이다. GitSearch의 토큰을 GitSearch만 읽는 이유다. 위젯과 익스텐션은 별도 프로세스에 별도 샌드박스라 본 앱의 기본 그룹이 보이지 않는다. 공유하려면 두 단계가 필요하다.
...
위젯이나 Notification Service Extension이 인증 API를 직접 호출하거나, 같은 Team ID의 다른 앱 사이에서 SSO를 할 때 쓴다. entitlement 없이 그룹을 지정하면 `errSecMissingEntitlement`(-34018)가 난다. 같은 Team ID끼리만 가능해서 타사 앱과는 원천적으로 안 된다.

위젯이라고 무조건 키체인을 공유해야 하는 건 아니다. 위젯이 본 앱이 만들어 둔 표시 데이터만 보여 준다면 App Group 공유 컨테이너로 충분하다. `UserDefaults(suiteName:)`이나 공유 파일 말이다. 축이 다르다. 일반 데이터 공유는 App Group, 자격증명 공유는 Keychain access group이다. 그리고 위젯이 잠금 중 타임라인을 갱신할 수 있으므로, 공유한 토큰의 접근성이 WhenUnlocked면 위젯 갱신이 침묵 실패한다. 앞에서 본 함정이 위젯에서 재현되는 자리다.

### kSecAttrAccessControl - 읽는 순간 인증을 요구한다

`kSecAttrAccessible`의 강화판이다. 접근성을 포함하면서 인증 요구를 얹기 때문에 둘 중 하나만 쓴다.
...
이후 `SecItemCopyMatching`이 시스템 Face ID 시트를 띄우는 블로킹 호출이 된다. 통과해야 Data가 나온다.
...
송금 직전 재인증, 비밀번호 관리자 항목 열람, 앱 잠금 마스터 키처럼 꺼내는 행위가 드물고 민감한 값에만 쓴다.

토큰에 안 쓰는 이유는 분명하다. 토큰은 요청마다 인터셉터가 조용히 읽는 값이라, 매 API 호출마다 Face ID가 뜨면 앱이 성립하지 않는다. 더 안전한데 생략한 게 아니라 용도가 다른 축이다.

---

## 정리

토큰은 Keychain에 `kSecClassGenericPassword`로 저장하고 service와 account 조합을 키로 쓴다. UserDefaults는 평문 plist라 자격증명에는 부적합하다.

접근성은 `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`다. 재부팅 후 첫 잠금해제 이후엔 백그라운드 네트워킹이 되면서, 백업과 기기 이전에는 따라가지 않아 새 기기에서 재로그인을 강제한다.

갱신은 저빈도 작업이라 SecItemUpdate 분기 대신 delete 후 add로 단순화했다. 실무 코드라면 OSStatus를 확인해 저장 실패를 침묵시키지 않는 것까지가 완성이다.

앱 삭제 후 키체인 잔존은 보장된 동작이 아니다. 재설치를 로그아웃으로 처리해야 한다면 첫 실행 감지로 명시적으로 비운다.

몇 가지를 덧붙인다.

Keychain의 암호화는 Data Protection 체계를 따른다. 파일 암호화와 같은 클래스 키 계층을 쓰고 Secure Enclave가 키를 보호한다. 접근성 상수가 곧 보호 클래스 선택이다.

WhenUnlocked가 더 안전한데 안 쓴 이유는 잠금 중 백그라운드 요청이 실패하기 때문이다. 보안 강도가 아니라 동작 요구와의 매칭 문제다.

토큰의 iCloud 동기화는 `kSecAttrSynchronizable`로 가능하지만 ThisDeviceOnly와 배타적이다. 기기에 묶기를 포기하는 결정이라 자격증명에는 보통 쓰지 않는다.

생체 보호까지 걸려면 SecAccessControl에 `biometryCurrentSet`을 쓴다. 읽기 시점에 Face ID를 요구하고 생체를 재등록하면 무효화된다.
...
- 표기 주의: 앱 삭제 후 잔존은 공식 문서화되지 않은 통상 동작이라 보장으로 취급하면 안 된다
