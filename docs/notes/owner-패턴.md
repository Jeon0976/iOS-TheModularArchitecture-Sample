...
모든 ViewModel과 VC가 `.sink { [weak self] in guard let self else ... }`와 `task.replace(onError: { [weak self] ... }) { [weak self] ... }`를 복붙하고 있었다. RxSwift 스타일의 `Publisher+SinkWith` 구현을 이식해 이 보일러플레이트를 걷어냈다.

이름을 붙이자면 RxSwift 6의 withUnretained 패턴이다. `subscribe(with:)`를 Combine의 `sink(with:)`로 그대로 옮긴 것이다. `[weak self]`와 guard를 확장 안에 가두고, 호출부에는 살아 있음이 보장된 `owner`를 파라미터로 넘긴다. 메모리 동작은 바뀌지 않는다. 여전히 weak 캡처에 guard고, 문법만 줄었지 참조 관계는 그대로다.
...
이 코드는 모든 ViewModel과 VC가 매 sink마다, 매 Task마다 반복한다. 사람이 지키는 규칙이라 언젠가 빠뜨린다. `ManagedTask`가 만들어진 이유와 같은 부류의 문제다.
...
[`Publisher+SinkWith.swift`](../../Projects/Core/CoreDesignSystem/Sources/Extensions/Publisher+SinkWith.swift)에 CoreDesignSystem의 확장으로 넣었다.
...
`Failure == Never` 전용으로 만든 이유가 있다. 이 앱은 에러를 Combine completion으로 흘리지 않고 상태나 이벤트 Subject로 전달한다. 그래서 모든 Publisher가 `Failure == Never`고 `receiveCompletion`을 받는 오버로드를 만들 필요가 없었다. RxSwift의 `subscribe(with:)`에 `onError:`가 있는 것은 Rx 스트림이 에러로 끝날 수 있기 때문인데, 이 앱에는 그 경우 자체가 없다.
...
onError와 operation 양쪽 모두 owner를 weak로 잡아 넘긴다. non-owner `start`에 위임하므로 `generation` 번호나 취소 침묵 같은 기존 규칙은 하나도 바뀌지 않는다.
...
`subscribe`와 `sink`, `disposeBag`과 `cancellables`, `disposed(by:)`와 `store(in:)`이 짝을 이룬다. 이름만 다르고 동작 규칙은 같다. `sink(with:)`는 RxSwift `subscribe(with:)`를 Combine으로 그대로 옮긴 것이다. RxSwift에는 스트림 중간에서 객체와 값의 튜플로 바꿔 주는 `withUnretained` 연산자도 따로 있는데(객체가 해제되면 스트림이 종료된다), 그 형태는 가져오지 않았다.

`ManagedTask(with:)`는 RxSwift 범위 밖이다. Task는 RxSwift가 아니라 Swift 구조적 동시성이다. owner를 주입한다는 같은 아이디어를 RxSwift가 다루지 않는 Task 클로저까지 넓힌 것이 이 이식의 확장분이다.
...
owner로 바뀌는 것은 self 멤버뿐이다. 클로저 밖의 지역 변수인 `query`나, 취소 후 반영을 막는 `guard !Task.isCancelled`는 그대로 유지된다.

---

## 왜 Swift 6 strict concurrency에서 컴파일되나

`Owner: AnyObject`는 Sendable이 아니다. 그런데도 제네릭 오버로드가 통과한다.

`ManagedTask`가 `@MainActor`이고 `operation`이 `@MainActor (Owner) async throws -> Void`다. owner를 만지는 클로저가 전부 MainActor에 격리돼 있다는 뜻이다. `start`의 `Task { }`도 `@MainActor` 컨텍스트에서 생성되므로 MainActor 실행자를 물려받아 operation이 메인에서 돈다.

따라서 Sendable이 아닌 `owner`가 격리 경계를 넘는 일이 없다. `[weak owner]`로 캡처해서 MainActor 안에서만 쓴다. Swift 6이 Sendable을 요구하는 것은 값이 액터 사이를 건널 때뿐이라 여기서는 그 요구가 발생하지 않는다.

관건은 `operation`의 `@MainActor`다. 이게 없거나 `Task.detached`였다면 Sendable이 아닌 owner가 다른 격리 영역으로 넘어가게 되어 컴파일 에러가 났을 것이다.
...
메모리 동작은 그대로다. 여전히 `[weak owner]`에 guard고, owner는 클로저가 실행되는 동안만 강참조로 잡혔다 풀린다. `guard let self`와 완전히 같다. 순환 참조도 없다. 취소와 `generation` 방어도 non-owner `start`에 위임하므로 그대로다.

문법이 줄었을 뿐 새로운 수명 관리 방식이 아니다. `[weak self]`가 사라진 게 아니라 API 안쪽으로 이사한 것이다.

---

## 어디에 쓰고 어디에 안 쓰나

**잘 맞는 곳.** `sink`와 Task operation, onError처럼 반환값이 없는 마지막 단계의 클로저다. owner가 해제됐으면 그냥 건너뛴다는 처리로 충분하다.

**안 맞는 곳.** `filter`처럼 값을 돌려주는 연산자다. owner가 해제됐을 때 true를 반환할지 false를 반환할지가 정해지지 않는다. 그래서 `.filter { [weak self] in self?.paginator.hasNextPage ?? false }` 한 곳은 일부러 바꾸지 않았다. `?? false`가 그 답을 이미 코드로 보여 준다.

이식 결과는 sink 14곳에 ManagedTask 6곳, 모두 20곳 전환이다. 남은 `[weak self]` 4곳은 전부 sink나 Task가 아닌 UIKit 콜백이다. DiffableDataSource cellProvider, `textFieldShouldSearch`, 알럿 액션, 그리고 위의 filter 하나다.

덧붙이면, 클로저가 self를 캡처하는 대신 파라미터로 전달받는 방향은 Apple API에도 있다. iOS 17의 `registerForTraitChanges(_:handler:)`가 핸들러 첫 인자로 `(self: Self, _)`를 넘겨주는 것이 같은 발상이다.
